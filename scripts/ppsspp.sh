	...
	if [[ "$var" == "ppsspp" ]] && [[ "$bitness" == "64" ]]; then
	 ...
	 if [[ ! -z "$ppsspp_patches" ]]; then
	  ...
	  cmake -DUSING_EGL=OFF \
		...
		-DCMAKE_C_FLAGS="-Ofast -fno-tree-slp-vectorize -D_NDEBUG -march=armv8-a+crc -mtune=cortex-a53 -ftree-vectorize -funsafe-math-optimizations -fno-stack-protector -U_FORTIFY_SOURCE" \
		-DCMAKE_CXX_FLAGS="-Ofast -fno-tree-slp-vectorize -D_NDEBUG -march=armv8-a+crc -mtune=cortex-a53 -ftree-vectorize -funsafe-math-optimizations -fpermissive -fno-stack-protector -U_FORTIFY_SOURCE" \
		-DUSE_MINIUPNPC=OFF \
		-DUSING_QT_UI=OFF \
		-DUSE_DISCORD=OFF ../.
	  make -j$(nproc)
